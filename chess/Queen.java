...
public class Queen extends Piece{
    ...
    public boolean isLegitMove(int i0, int j0, int i1, int j1){
        ...
        int step_j=0;
        int in,jn;
        if(delta_i!=0 && delta_j!=0 &Math.abs(delta_i)==Math.abs(delta_j)){
        ...
        if ((((delta_i!=0 && delta_j==0)^(delta_i==0 && delta_j!=0)) && squared_distance <= 7)
            || (Math.abs(delta_i)==Math.abs(delta_j) && squared_distance <= 2*7*7)){
            while(true){
        ...
        //System.out.println("Size: "+possible_moves.size());
        ...
        //         ||(board[i1][j1].getPiece().colour != colour && Math.abs(delta_i)==Math.abs(delta_j) && squared_distance <= 2*7*7)){
        //         return true;
        //     }else{
        //         return false;
        //     }
